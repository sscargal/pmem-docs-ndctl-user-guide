...
{% hint style="info" %}
Note: These security features require ndctl v64.0 or later and requires Linux Kernel 5.0 or later. Vendor specific utilities may also provide the same functionality on earlier Linux Kernel versions. For Intel\(R\) Optane\(TM\) Persistent Memory Modules, see the '[ipmctl](https://github.com/intel/ipmctl)' utility.
{% endhint %}
...
The command supports two different methods of performing the cryptographic erase. The default is `crypto-erase`, but additionally, an [overwrite ](managing-nvdimm-security.md#overwrite)option is available which overwrites not only the data area, but also the label area, thus losing record of any namespaces the given NVDIMM participates in.
